// Studio Chatbot – lógica principal
(function(){
  'use strict';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    on(examplesList, 'click', (e)=>{
      ...
      if(del!==null){ state.examples.splice(+del,1); renderExamples(); toast('Ejemplo eliminado','warn'); autoTrainIfData(); return; }
      if(edit!==null){
        ...
        renderExamples(); toast('Ejemplo actualizado','ok'); autoTrainIfData();
      }
    });
    ...
    function addExample(){
      const uEl = $('exUser'), aEl = $('exBot');
      const u = uEl?.value.trim();
      const a = aEl?.value.trim();
      if(!u || !a){ alert('Completa ambos campos'); return; }
      state.examples.push({user:u, assistant:a});
      if(uEl) uEl.value=''; if(aEl) aEl.value='';
      renderExamples();
      autoTrainIfData();
      toast('Ejemplo agregado y entrenado','ok');
    }
    ...
    async function importFile(){
      ...
      try{
        ...
        renderExamples();
        autoTrainIfData();
        toast(`Importados ${cleaned.length} ejemplos y modelo entrenado`,'ok');
      }catch(err){
