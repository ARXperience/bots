...
// ====== Render con Markdown + botón Copiar + AUTOSCROLL ROBUSTO ======
function render(role, mdText){
  ...
  // 🔽 AUTOSCROLL robusto (desktop + móvil)
  autoScroll();
...
function autoScroll() {
  ...
  // 2) Tras el próximo frame (cuando el DOM ya pintó)
  requestAnimationFrame(() => {
    msgs.scrollTop = msgs.scrollHeight;
  });

  // 3) Fallback para móviles (iOS Safari) y fuentes/lazy content
  setTimeout(() => {
    msgs.scrollTop = msgs.scrollHeight;
